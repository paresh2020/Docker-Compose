...
if test -f "$BACKUP_DIR"; then
docker exec flaskdockercomposepipeline_db_1 bash -c 'exec mysqldump  -h $DOCKER_MYSQL_SERVICE_NAME -P "$PORT" -u "$MYSQL_USER" -p --all-databases' > gzip > "$BACKUP_DIR"/"$BACKUP_FILE_NAME" || exit 0;
fi
